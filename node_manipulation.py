...
def get_node_ID(workflow, title):
    ...
    if len(matching_ids) > 1:
        print(f"node_manipulation.get_node_ID(): Error - Found duplicate titles: {matching_ids}")
        raise ValueError(f"Duplicate titles found for '{title}': {matching_ids}")
    
    if not matching_ids:
        print(f"node_manipulation.get_node_ID(): No node found with title: {title}")
        return None
        
    print(f"node_manipulation.get_node_ID(): Found node ID: {matching_ids[0]}")
...
def set_KSampler(workflow, nodeTitle, seed, steps=20, cfg=7, sampler_name='dpmpp_2m', scheduler='karras', denoise=1):
    ...
    print(f"node_manipulation.set_KSampler(): Parameters - seed={seed}, steps={steps}, cfg={cfg}, sampler={sampler_name}, scheduler={scheduler}, denoise={denoise}")
    ...
    if node_id is not None:
        ...
        print(f"node_manipulation.set_KSampler(): Successfully configured node {node_id}")
    else:
        print(f"node_manipulation.set_KSampler(): Failed - Node '{nodeTitle}' not found")
...
def set_positive_prompt(workflow, ckpt_name, lora_names, nodeTitle="Positive", style_name=None):
    ...
    print(f"node_manipulation.set_positive_prompt(): Parameters - checkpoint={ckpt_name}, loras={lora_names}, style={style_name}")
    ...
    if node_id is not None:
        ...
        print(f"node_manipulation.set_positive_prompt(): Successfully set prompt for node {node_id}")
        print(f"node_manipulation.set_positive_prompt(): Prompt: {positive_prompt}")
    else:
        print(f"node_manipulation.set_positive_prompt(): Failed - Node '{nodeTitle}' not found")
...
def set_negative_prompt(workflow, ckpt_name, lora_names, nodeTitle="Negative", style_name=None):
    ...
    if node_id is not None:
        ...
        print(f"node_manipulation.set_negative_prompt(): Node with title '{nodeTitle}' not found.")
...
def set_lora(workflow, nodeTitle, lora_name, strength_model=1, strength_clip=1):
    ...
    print(f"node_manipulation.set_lora(): Parameters - lora={lora_name}, model_strength={strength_model}, clip_strength={strength_clip}")
    ...
    if node_id is not None:
        ...
        print(f"node_manipulation.set_lora(): Successfully configured node {node_id}")
    else:
        print(f"node_manipulation.set_lora(): Failed - Node '{nodeTitle}' not found")
...
def set_resolution(workflow, nodeTitle, width, height):
    ...
    print(f"node_manipulation.set_resolution(): Parameters - width={width}, height={height}")
    ...
    if node_id is not None:
        ...
        print(f"node_manipulation.set_resolution(): Successfully set resolution {width}x{height} for node {node_id}")
    else:
        print(f"node_manipulation.set_resolution(): Failed - Node '{nodeTitle}' not found")
